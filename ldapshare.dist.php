...
class ldapshare {
	public function __construct() {
		...
		$this->select_post = 'SELECT post.*, user.*, DATE_ADD(post.post_datecreated, INTERVAL '.$_SESSION['ldapshare']['data']['timezone'].' HOUR) AS post_datecreated, (SELECT COUNT(DISTINCT(comment.comment_id)) FROM '.TABLE_COMMENT.' comment WHERE comment.post_id = post.post_id) AS count_comment, (SELECT COUNT(DISTINCT(link.link_id)) FROM '.TABLE_LINK.' link WHERE link.post_id = post.post_id) AS post_countlink, (SELECT COUNT(DISTINCT(l.like_id)) FROM '.TABLE_LIKE.' l WHERE l.post_id = post.post_id) AS post_countlike, IF(l_you.like_id IS NOT NULL, 1, 0) AS you_like FROM '.TABLE_POST.' post LEFT JOIN '.TABLE_USER.' user ON user.user_id = post.user_id LEFT JOIN '.TABLE_LIKE.' l_you ON l_you.post_id = post.post_id AND l_you.user_id = :user_id';
